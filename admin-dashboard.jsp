...
<%
    ...
    try {
        ...
        rs = stmt.executeQuery("SELECT NVL(SUM(total_amount), 0) as total FROM orders");
        if(rs.next()) totalRevenue = rs.getDouble("total");
        
        // Today's Revenue
        rs = stmt.executeQuery("SELECT NVL(SUM(total_amount), 0) as total FROM orders WHERE TRUNC(order_date) = TRUNC(SYSDATE)");
        if(rs.next()) todayRevenue = rs.getDouble("total");
        
        // This Month Revenue
        rs = stmt.executeQuery("SELECT NVL(SUM(total_amount), 0) as total FROM orders WHERE EXTRACT(MONTH FROM order_date) = EXTRACT(MONTH FROM SYSDATE) AND EXTRACT(YEAR FROM order_date) = EXTRACT(YEAR FROM SYSDATE)");
        if(rs.next()) thisMonthRevenue = rs.getDouble("total");
        
        // Pending Revenue (orders not yet delivered)
        rs = stmt.executeQuery("SELECT NVL(SUM(total_amount), 0) as total FROM orders WHERE status IN ('Pending', 'Preparing')");
        if(rs.next()) pendingRevenue = rs.getDouble("total");
        
        // Average Order Value
        rs = stmt.executeQuery("SELECT NVL(AVG(total_amount), 0) as avg_val FROM orders");
        if(rs.next()) averageOrderValue = rs.getDouble("avg_val");
    ...
    <div class="main-content">
        ...
        <div class="revenue-highlight">
            <div class="revenue-title">
                <i class="fa-thin fa-circle-bangladeshi-taka"></i> Revenue from Food Sales
            </div>
            <div class="revenue-grid">
                <div class="revenue-item">
                    <div class="revenue-label"><i class="fas fa-coins"></i> Total Revenue</div>
                    <div class="revenue-amount">BDT<%= String.format("%,.2f", totalRevenue) %></div>
                    <div class="revenue-subtext">All-time earnings</div>
                </div>
                
                <div class="revenue-item">
                    <div class="revenue-label"><i class="fas fa-calendar-day"></i> Today's Revenue</div>
                    <div class="revenue-amount">BDT<%= String.format("%,.2f", todayRevenue) %></div>
                    <div class="revenue-subtext">Sales today</div>
                </div>
                
                <div class="revenue-item">
                    <div class="revenue-label"><i class="fas fa-calendar-alt"></i> This Month</div>
                    <div class="revenue-amount">BDT<%= String.format("%,.2f", thisMonthRevenue) %></div>
                    <div class="revenue-subtext">Current month revenue</div>
                </div>
                
                <div class="revenue-item">
                    <div class="revenue-label"><i class="fas fa-clock"></i> Pending Orders</div>
                    <div class="revenue-amount">BDT<%= String.format("%,.2f", pendingRevenue) %></div>
                    <div class="revenue-subtext">In progress orders</div>
                </div>
                
                <div class="revenue-item">
                    <div class="revenue-label"><i class="fas fa-chart-bar"></i> Avg Order Value</div>
                    <div class="revenue-amount">BDT<%= String.format("%,.2f", averageOrderValue) %></div>
                    <div class="revenue-subtext">Per transaction</div>
                </div>
            </div>
        </div>
